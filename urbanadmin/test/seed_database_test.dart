...
void main() {
  test('Seed Database', () async {
    // We need to initialize Firebase manually for the test environment
    ...
    debugPrint('🚀 Wiping existing portfolio...');
    final services = await firestore.collection('services').get();
    for (var doc in services.docs) {
      await doc.reference.delete();
      debugPrint('🗑️ Deleted: ${doc.id}');
    }

    debugPrint('🌱 Seeding fresh catalog...');
    ...
    for (var cat in categoriesToSeed) {
      ...
      debugPrint('✅ Seeded: ${cat['categoryName']} with ${subServices.length} sub-services.');
    }
